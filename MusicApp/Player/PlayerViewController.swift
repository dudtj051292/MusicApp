...
class PlayerViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        timeObserver = simplePlayer.addPerioidicTimeObserver(forInterval: CMTime(seconds: 1, preferredTimescale: 10), queue: DispatchQueue.main) { time in
...
extension PlayerViewController {
    ...
    func updateTime(time: CMTime) {
        ...
        totalDurationLabel.text = secondsToString(sec: simplePlayer.totalDuration)  // 39.2045  >> 00:39
        
        if isSeeking == false {
            // 노래 들으면서 시킹하면, 자꾸 슬라이더가 업데이트 됨, 따라서 시킹아닐때마 슬라이더 업데이트하자
            timeSlider.value = Float(simplePlayer.currentTime/simplePlayer.totalDuration)
        }
    }
